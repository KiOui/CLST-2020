...
from processes.services import parse_profiles, parse_parameters, handle_parameter_changing, handle_profile_changing, \
    autoconfigure_file_settings, check_process_ready, find_ready_profiles
...
class ProcessRetrieveAPIView(RetrieveAPIView):
    ...
    def get_queryset(self):
        return Process.objects.filter(project__in=Project.objects.filter(user=self.request.user))
...
class ScriptSettingsRetrieveUpdateAPIView(RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project.pipeline.fa_script == script or project.pipeline.g2p_script == script:
            ...
            if not partial:
                ParameterSetting.objects.filter(project=project, base_parameter__corresponding_script=script).exclude(
                    pk__in=[x.pk for x in parameters_altered]).delete()
                FileSetting.objects.filter(file__project=project, input_template__corresponding_profile__script=script).exclude(
...
def process_file_list_view(request, **kwargs):
    ...
    if request.user.is_authenticated and request.user == process.project.user:
        if request.method == "GET":
            return Response(status=status.HTTP_200_OK, data=[{"file": x, "preset": OutputTemplate.match_any(x, process.script.output_templates)} for x in process.file_list])
        elif request.method == "POST":
            if 'files' in request.data.keys() and type(request.data['files']) == list:
                sanitized_files = [x for x in request.data['files'] if type(x) == str]
                process.move_downloaded_output_files(files=sanitized_files)
                return Response(status=status.HTTP_200_OK)
            else:
                raise ValidationError(detail={'files': "A list of strings is required."})
        else:
...
def start_process(request, **kwargs):
    ...
    if request.user.is_authenticated and request.user == project.user:
        if script == project.pipeline.fa_script or script == project.pipeline.g2p_script and (profile is None or profile.script == script):
            errors = check_process_ready(project, script, profile)
            if len(errors) > 0:
                return Response(status=status.HTTP_428_PRECONDITION_REQUIRED, data={"errors": errors})
            else:
                if profile is None:
                    ...
                    if len(profiles_ready) == 1:
                        ...
                        return Response(status=status.HTTP_428_PRECONDITION_REQUIRED, data={"errors": ["There is no profile that can be applied to this project, please start a process manually. This can be done on either the Forced Alignment step (for FA processes) or the Grapheen to Phoneem step (for G2P processes)."]})
                    else:
                        return Response(status=status.HTTP_428_PRECONDITION_REQUIRED, data={"errors": ["There are multiple profile that can be applied to this project, please start a process manually. This can be done on either the Forced Alignment step (for FA processes) or the Grapheen to Phoneem step (for G2P processes)."]})
                process = Process.objects.create(script=script, project=project)
                try:
                    process.start_safe(profile, {x.base_parameter.name: x.value for x in ParameterSetting.objects.filter(project=project, base_parameter__corresponding_script=script)})
                except BaseParameter.ParameterException as e:
                    return Response(status=status.HTTP_500_INTERNAL_SERVER_ERROR, data={"errors": [e]})
                return Response(status=status.HTTP_200_OK, data={"errors": [], "process": process.id, "redirect": reverse("processes:process-detail", kwargs={"process": process})})
        else:
            return Response(status=status.HTTP_404_NOT_FOUND)
    else:
        raise PermissionDenied
